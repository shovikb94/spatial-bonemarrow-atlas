...
All.combined_mapped_blastslabeled <- readRDS("/mnt/isilon/tan_lab_imaging/Analysis/bandyopads/NBM_CODEX_Atlas/Combined_Analysis/Seurat/ReferenceMap_AML_Step6/objects/All.combined_mapped_unlabeled_040423.RDS")
annotations_col <- "classified_cluster_anno_l2"
integrated_seurat <- All.combined_mapped_blastslabeled
cluster_factor <- as.numeric(as.factor(integrated_seurat$classified_cluster_anno_l2))
integrated_seurat$cluster_levels <- cluster_factor
conversion_table <- data.frame(
  Level = levels(as.factor(integrated_seurat$classified_cluster_anno_l2)),
...
for(name in sample_names){
  ...
  write.csv(annotations_df, file = paste0("/mnt/isilon/tan_lab_imaging/Analysis/bandyopads/NBM_CODEX_Atlas_NeighborhoodMasks/Masks_ForManuscript/AMLandNSM_ClusterMask_CSVs/Unfiltered_",name,"_",annotations_col,"_","_10252023.csv"))
}
