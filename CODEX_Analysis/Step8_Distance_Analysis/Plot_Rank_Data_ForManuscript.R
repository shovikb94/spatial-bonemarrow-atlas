...
# Calculate meta p-values using Fisher's Method -----
metap <- ranks %>% group_by(celltype, structure) %>% summarise(meta_p = sumlog(pvalue+0.00000000000000001, log.p = FALSE)[['p']])
metap_wide <- dcast(metap, celltype ~ structure, value.var = "meta_p")

to_plot <- left_join(summarized_ranks, metap, by = c("celltype" = "celltype", "structure" = "structure"))
to_plot$sig <- to_plot$meta_p < 0.05
