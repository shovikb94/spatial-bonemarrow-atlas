### NBM37_H35 Seurat Object Generation Script
### This script takes as input a channel_names text file and the cell X protein output of our Mesmer pipeline and outputs a Seurat Object for NBM37
...
file_path = '/mnt/isilon/tan_lab_imaging/FUSION/NBM37_H35_citrate/H35/Scan1/mesmercombined_markers.csv'
...
gc_csd_raw <- gc_csd_raw %>% dplyr::filter(x.coord < 14000, y.coord >4000, x.coord > 4500, y.coord < 14000)
cells_to_remove <- subset(gc_csd_raw, subset = x.coord > 8930 & y.coord < 6400 & x.coord < 9308 & y.coord > 5756) # Use this line to further crop out piece which had artifact
gc_csd_raw <- anti_join(gc_csd_raw, cells_to_remove, by = "CellID")
cells_to_remove <- subset(gc_csd_raw, subset = x.coord > 7268 & y.coord < 8168 & x.coord < 7762 & y.coord > 7778) # Use this line to further crop out piece which had artifact
gc_csd_raw <- anti_join(gc_csd_raw, cells_to_remove, by = "CellID")
cells_to_remove <- subset(gc_csd_raw, subset = x.coord > 8820 & y.coord < 12402 & x.coord < 9278 & y.coord > 11736) # Use this line to further crop out piece which had artifact
gc_csd_raw <- anti_join(gc_csd_raw, cells_to_remove, by = "CellID")
...
codex_H35 <- CreateSeuratObject(t_gc_csd, project = "SB67_NBM37_H35_CODEX_Mesmer", assay="CODEX", meta.data = gc_csd_raw)
saveRDS(codex_H35, "codex_H35_SeuratObj.RDS")
