...
gc_csd_raw <- gc_csd_raw %>% dplyr::filter(y.coord < 23000, x.coord < 20000, x.coord > 2500, y.coord > 0)
...
codex_assay <- CreateAssayObject(counts = t_gc_csd, project = "NBM70_H10_Citrate_CODEX", assay="CODEX", meta.data = gc_csd_raw)
codex_seurat <- CreateSeuratObject(t_gc_csd, project = "NBM70_H10_Citrate_CODEX", assay="CODEX", meta.data = gc_csd_raw)
codex_seurat[["CODEX"]] = codex_assay
saveRDS(codex_seurat, "/mnt/isilon/tan_lab/sussmanj/CODEX/NBM_Receptor_Ligand/Seurat_Objects/NBM70_H10_Citrate_CODEX_SeuratObj.RDS")
