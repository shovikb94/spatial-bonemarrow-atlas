...
gc_csd_raw <- gc_csd_raw %>% dplyr::filter(y.coord < 41000, x.coord < 15000, x.coord > 0, y.coord > 23000)
...
codex_assay <- CreateAssayObject(counts = t_gc_csd, project = "NBM70_H26_Citrate_CODEX", assay="CODEX", meta.data = gc_csd_raw)
codex_seurat <- CreateSeuratObject(t_gc_csd, project = "NBM70_H26_Citrate_CODEX", assay="CODEX", meta.data = gc_csd_raw)
codex_seurat[["CODEX"]] = codex_assay
saveRDS(codex_seurat, "/mnt/isilon/tan_lab/sussmanj/CODEX/NBM_Receptor_Ligand/Seurat_Objects/NBM70_H26_Citrate_CODEX_SeuratObj.RDS")
