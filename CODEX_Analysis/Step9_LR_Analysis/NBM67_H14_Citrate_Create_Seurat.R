...
gc_csd_raw <- gc_csd_raw %>% dplyr::filter(y.coord < 65000, x.coord < 20000, x.coord > 0, y.coord > 40000)
...
codex_assay <- CreateAssayObject(counts = t_gc_csd, project = "NBM67_H14_Citrate_CODEX", assay="CODEX", meta.data = gc_csd_raw)
codex_seurat <- CreateSeuratObject(t_gc_csd, project = "NBM67_H14_Citrate_CODEX", assay="CODEX", meta.data = gc_csd_raw)
codex_seurat[["CODEX"]] = codex_assay
saveRDS(codex_seurat, "/mnt/isilon/tan_lab/sussmanj/CODEX/NBM_Receptor_Ligand/Seurat_Objects/NBM67_H14_Citrate_CODEX_SeuratObj.RDS")
