...
gc_csd_raw <- gc_csd_raw %>% dplyr::filter(y.coord < 13000, x.coord < 15000, x.coord > 2000, y.coord > 0)
...
codex_assay <- CreateAssayObject(counts = t_gc_csd, project = "NBM67_H38_Citrate_CODEX", assay="CODEX", meta.data = gc_csd_raw)
codex_seurat <- CreateSeuratObject(t_gc_csd, project = "NBM67_H38_Citrate_CODEX", assay="CODEX", meta.data = gc_csd_raw)
codex_seurat[["CODEX"]] = codex_assay
saveRDS(codex_seurat, "/mnt/isilon/tan_lab/sussmanj/CODEX/NBM_Receptor_Ligand/Seurat_Objects/NBM67_H38_Citrate_CODEX_SeuratObj.RDS")
