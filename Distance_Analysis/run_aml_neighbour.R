...
foreach (s = 1:3) %dopar% {
    ...
    # Save to text files
    output_dir <- "output_f/aml_"

    writeLines(as.character(sample_list), paste0(output_dir, "sample_list_", structure, '.txt'))
    writeLines(as.character(celltype_list), paste0(output_dir, "celltype_list_", structure, '.txt'))
    writeLines(as.character(coefficients_list_1), paste0(output_dir, "coefficients_list_1_", structure, '.txt'))
    writeLines(as.character(coefficients_list_2), paste0(output_dir, "coefficients_list_2_", structure, '.txt'))
    writeLines(as.character(pop_list), paste0(output_dir, "pop_list_", structure, '.txt'))
    writeLines(as.character(distance_median_list), paste0(output_dir, "distance_median_list_", structure, '.txt'))
    writeLines(as.character(distance_sd_list), paste0(output_dir, "distance_sd_list_", structure, '.txt'))
    writeLines(as.character(pvalue_list), paste0(output_dir, "pvalue_list_", structure, '.txt'))
    writeLines(as.character(iqr_list), paste0(output_dir, "iqr_list_", structure, '.txt'))
}
