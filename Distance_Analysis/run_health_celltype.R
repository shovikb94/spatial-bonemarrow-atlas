...
foreach (s = 1:6) %dopar% {
    ...
    for (i in 1:length(sample_names)) {
        ...
        for (j in 1:length(unique_cell_types)) {
            ...
            # print(cell_type)
            coords_crop <- subset(sample_data_crop, subset = cluster_anno_l2 == cell_type)
    ...
    writeLines(sample_vector,paste0( "output_f/sample_list_", structure, '.txt'))
    celltype_vector <- as.character(celltype_list)
    writeLines(celltype_vector, paste0("output_f/celltype_list_", structure, '.txt'))
    coefficients_list_1_vector <- as.character(coefficients_list_1)
    writeLines(coefficients_list_1_vector, paste0("output_f/coefficients_list_1_", structure, '.txt'))
    coefficients_list_2_vector <- as.character(coefficients_list_2)
    writeLines(coefficients_list_2_vector, paste0("output_f/coefficients_list_2_", structure, '.txt'))
    pop_vector <- as.character(pop_list)
    writeLines(pop_vector, paste0("output_f/pop_list_", structure,  '.txt'))
    distance_median_vector <- as.character(distance_median_list)
    writeLines(distance_median_vector, paste0("output_f/distance_mean_list_", structure, '.txt'))
    distance_sd_vector <- as.character(distance_sd_list)
    writeLines(distance_sd_vector, paste0("output_f/distance_sd_list_", structure, '.txt'))
    pvalue_vector <- as.character(pvalue_list)
    writeLines(pvalue_vector, paste0("output_f/pvalue_list_", structure, '.txt'))
    iqr_vector <- as.character(iqr_list)
    writeLines(iqr_vector, paste0("output_f/iqr_list_", structure, '.txt'))
}   
